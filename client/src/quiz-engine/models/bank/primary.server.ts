...
const englishBank = () => [
  ...fromMap(
    opposites,
    (k) => `What is the opposite of "${k}"?`,
    (k, v) => `The opposite of "${k}" is "${v}".`,
  ),
  ...fromMap(
    plurals,
    (k) => `What is the plural of "${k}"?`,
    (k, v) => `The plural of "${k}" is "${v}".`,
  ),
  ...fromMap(
    pastTense,
    (k) => `What is the past tense of "${k}"?`,
    (k, v) => `The past tense of "${k}" is "${v}".`,
  ),
];
...
const organs: [string, string][] = [
  ...
  ["brain", "Controls thinking and the body"],
  ["stomach", "Breaks down the food we eat"],
  ["kidneys", "Filter waste out of the blood"],
  ["skin", "Protects the body and senses touch"],
  ["eyes", "Let us see light and colour"],
  ["ears", "Let us hear sound and keep balance"],
  ["bones", "Give the body shape and support"],
  ["muscles", "Help the body move"],
  ["teeth", "Cut and grind food"],
  ["tongue", "Helps us taste and speak"],
  ["liver", "Cleans the blood and stores energy"],
  ["nose", "Lets us smell and breathe"],
...
const animalGroups: [string, string][] = [
  ...
  ["A crab", "A crustacean"],
  ["A bat", "A mammal that flies"],
  ["A penguin", "A bird that swims"],
  ["A crocodile", "A reptile"],
  ["A dolphin", "A mammal that lives in water"],
  ["A bee", "An insect"],
  ["A tortoise", "A reptile"],
...
const scienceBank = () => [
  ...fromMap(
    organs,
    (k) => `What is the main job of the ${k}?`,
    (k, v) => `The ${k}: ${v.toLowerCase()}.`,
  ),
  ...fromMap(
    animalGroups,
    (k) => `${k} belongs to which group?`,
...
const helpers: [string, string][] = [
  ...
  ["A nurse", "Cares for patients in a clinic"],
  ["A carpenter", "Makes things out of wood"],
  ["A tailor", "Sews and repairs clothes"],
  ["A driver", "Transports people and goods"],
  ["A mechanic", "Repairs vehicles"],
  ["A pilot", "Flies an aeroplane"],
  ["A pharmacist", "Prepares and sells medicine"],
  ["A librarian", "Looks after books in a library"],
  ["A journalist", "Reports news stories"],
...
const socialBank = () => [
  ...fromMap(
    ...
    (k, v) => `${v} is the capital city of ${k}.`,
  ),
  ...fromMap(
    helpers,
    (k) => `What does ${k.toLowerCase()} do?`,
    (k, v) => `${k} ${v.toLowerCase()}.`,
  ),
  ...fromMap(
    civics,
...
const devices: [string, string][] = [
  ["A keyboard", "Types letters and numbers into a computer"],
  ["A mouse", "Points at and clicks items on screen"],
  ["A monitor", "Displays what the computer is doing"],
  ["A printer", "Puts computer work onto paper"],
  ["A scanner", "Copies paper documents into the computer"],
  ["A speaker", "Plays sound from the computer"],
  ["A microphone", "Records sound into the computer"],
  ["A webcam", "Captures video for calls"],
  ["A flash drive", "Stores and carries files"],
  ["A router", "Connects devices to the internet"],
  ["A CPU", "Processes the computer's instructions"],
  ["RAM", "Holds data the computer is using right now"],
  ["A hard disk", "Stores files even when the power is off"],
  ["A projector", "Shows the screen on a large wall"],
  ["A touchscreen", "Lets you control a device by touching it"],
];

const ictFacts: [string, string][] = [
  ["Which key makes a capital letter?", "Shift"],
  ["Which key deletes the letter before the cursor?", "Backspace"],
  ["Which key starts a new line?", "Enter"],
  ["Which key adds a space between words?", "Spacebar"],
