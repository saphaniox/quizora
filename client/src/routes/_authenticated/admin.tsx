...
function AdminPage() {
  ...
  const totalQuestions = levelsQuery.data?.totalQuestions ?? sections.reduce((sum, section) => sum + section.questionCount, 0);
  ...
  const largestSections = useMemo(() => {
    return sections.slice().sort((a, b) => b.questionCount - a.questionCount).slice(0, 5);
  }, [sections]);
  ...
  const qaFlags = useMemo(() => {
    ...
    const repeatedTitles = sections.filter((section, index, list) => {
      return list.findIndex((candidate) => candidate.title.toLowerCase() === section.title.toLowerCase()) !== index;
    });
  ...
  const readiness: ReadinessItem[] = [
    {
      ...
      detail: "Quiz payloads are generated by server handlers and public quiz responses omit answer keys.",
      tone: "ready",
    },
    {
      label: "Durable persistence",
      detail: "Standalone Fastify/PostgreSQL persistence exists, but local Postgres is not running and client server routes keep activity in memory.",
      tone: "blocked",
    },
    {
      label: "Admin authorization",
      detail: "User sessions exist, but there is no role table, admin middleware, or permission check for privileged writes yet.",
      tone: "blocked",
    },
    {
      label: "Content management API",
      detail: "Wait for authenticated create/update/publish endpoints with audit logs before enabling editing.",
      tone: "warning",
    },
  ];
    ...
    <div className="bg-background">
      <section className="border-b border-border bg-card">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-8 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8">
          <div>
            ...
            <h1 className="mt-4 text-3xl font-semibold tracking-tight text-foreground">Admin dashboard</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              Monitor catalogue coverage, learner activity, content quality, and backend gaps before enabling a full
              content studio.
            </p>
          </div>
          <button
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Check `VITE_API_URL`, the API process, and PostgreSQL availability if this persists.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard icon={Layers3} label="Learning levels" value={formatNumber(levels.length)} detail="Published catalogue groups" />
          <MetricCard icon={BookOpen} label="Sections" value={formatNumber(totalSections)} detail="Queryable quiz sections" />
          <MetricCard icon={FileQuestion} label="Questions" value={formatNumber(totalQuestions)} detail={`${formatNumber(averageQuestions)} average per section`} />
          <MetricCard icon={Trophy} label="Score entries" value={formatNumber(leaderboard.length)} detail={`${activity.passRate}% estimated pass rate`} />
        </div>
          ...
          <section className="rounded-lg border border-border bg-card">
            <div className="border-b border-border p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h2 className="text-base font-semibold text-card-foreground">Catalogue coverage</h2>
                  <p className="mt-1 text-sm text-muted-foreground">Live section inventory grouped by level and difficulty.</p>
                </div>
                  ...
                  {visibleSections.slice(0, 12).map((section) => (
                    <tr key={section.id}>
                      <td className="px-5 py-4">
                        ...
                        <p className="mt-1 line-clamp-1 max-w-md text-xs text-muted-foreground">{section.description}</p>
                      </td>
                      ...
                      <td className="px-5 py-4 text-right font-medium text-foreground">{formatNumber(section.questionCount)}</td>
                      <td className="px-5 py-4">
            ...
            {visibleSections.length === 0 && (
              <div className="p-10 text-center">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Adjust the search or level filter.</p>
              </div>
            )}
          </section>
          ...
          <div className="space-y-6">
                ...
                {difficultyCounts.map(({ difficulty, count }) => (
                  <div key={difficulty}>
                    ...
                    <div className="mt-2 h-2 rounded-full bg-muted">
                      <div className="h-2 rounded-full bg-primary" style={{ width: `${percent(count, totalSections)}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="rounded-lg border border-border bg-card p-5">
              ...
              <div className="mt-4 space-y-3">
                {readiness.map((item) => (
                  <div key={item.label} className={cn("rounded-lg border p-3", statusClass(item.tone))}>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-3">
          ...
          <section className="rounded-lg border border-border bg-card p-5">
            ...
            <div className="mt-4 space-y-3">
              {qaFlags.map((flag) => (
                <div key={flag.label} className="flex items-start justify-between gap-4 rounded-md border border-border bg-background p-3">
                  <div>
                    <p className="text-sm font-medium text-foreground">{flag.label}</p>
                    <p className="mt-1 text-xs leading-5 text-muted-foreground">{flag.detail}</p>
                  </div>
                  <span className={cn("rounded-md border px-2 py-1 text-xs font-semibold", statusClass(flag.tone))}>
                    {flag.value}
                  </span>
                </div>
              ))}
          ...
          <section className="rounded-lg border border-border bg-card p-5">
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              <ArchitectureRow label="Quiz API" value="Server routes provide public quiz payloads" tone="ready" />
              <ArchitectureRow label="Answers" value="Kept server-side until submission" tone="ready" />
              <ArchitectureRow label="Persistence" value="Needs running PostgreSQL for production" tone="blocked" />
            </div>
          </section>
        </div>
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-5">
            {largestSections.map((section) => (
              <div key={section.id} className="rounded-lg border border-border bg-background p-4">
                <p className="line-clamp-2 min-h-10 text-sm font-semibold text-foreground">{section.title}</p>
                <p className="mt-2 text-xs text-muted-foreground">{section.levelName}</p>
                <p className="mt-4 text-2xl font-semibold text-foreground">{formatNumber(section.questionCount)}</p>
              </div>
            ))}
          </div>
        </section>
...
function summarizeActivity(entries: LeaderboardEntry[]) {
  ...
  const averageScore = total ? Math.round(entries.reduce((sum, entry) => sum + entry.percentage, 0) / total) : 0;
  ...
  const recent = entries.filter((entry) => new Date(entry.completedAt).getTime() >= recentThreshold).length;
...
function ArchitectureRow({ label, value, tone }: { label: string; value: string; tone: StatusTone }) {
  return (
