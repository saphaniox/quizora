  ...
  generate(
    ...
    (index, random) => {
      ...
      if (kind === 1) {
        ...
        return draft(`A rectangle measures ${a} cm by ${b} cm. What is its perimeter?`, `${perimeter} cm`, numericOptions(perimeter, 6).map((v) => `${v} cm`), `Perimeter = 2(${a} + ${b}) = ${perimeter} cm.`);
      }
      if (kind === 2) {
        const area = (a * b) / 2;
...
const elements: [string, string][] = [
  ...
  ["Sulphur", "S"], ["Chlorine", "Cl"], ["Potassium", "K"], ["Calcium", "Ca"], ["Iron", "Fe"],
  ["Copper", "Cu"], ["Zinc", "Zn"], ["Silver", "Ag"], ["Gold", "Au"], ["Lead", "Pb"],
  ["Mercury", "Hg"], ["Tin", "Sn"], ["Neon", "Ne"], ["Argon", "Ar"], ["Uranium", "U"],
];
...
const chemistryBank = () => [
  ...fromMap(elements, (k) => `What is the chemical symbol for ${k}?`, (k, v) => `The symbol for ${k} is ${v}.`),
  ...fromMap(chemistryFacts, (k) => k, (_k, v) => `The correct answer is: ${v}.`),
];
...
const physicsBank = () => [
  ...fromMap(physicsFacts, (k) => k, (_k, v) => `The correct answer is: ${v}.`),
  ...generate(
    30,
    (index, random) => {
      const kind = index % 3;
...
const biologyFacts: [string, string][] = [
  ...
  ["Which process makes gametes?", "Meiosis"], ["How many chromosomes are in a human body cell?", "46"],
  ["What is a change in DNA called?", "A mutation"],
];

const biologyBank = () => [
  ...fromMap(biologyFacts, (k) => k, (_k, v) => `The correct answer is: ${v}.`),
  ...fromMap(
    [
      ["heart", "Pumps blood through the body"], ["kidney", "Filters blood and makes urine"],
      ["liver", "Detoxifies blood and stores glycogen"], ["lung", "Exchanges oxygen and carbon dioxide"],
      ["small intestine", "Absorbs digested nutrients"], ["large intestine", "Absorbs water from waste"],
      ["skin", "Protects the body and regulates temperature"], ["stomach", "Digests protein using acid and enzymes"],
      ["spinal cord", "Carries nerve signals to and from the brain"], ["ovary", "Produces eggs and female hormones"],
      ["testis", "Produces sperm and testosterone"], ["thyroid", "Controls the rate of metabolism"],
      ["spleen", "Filters blood and stores white blood cells"], ["bone marrow", "Makes blood cells"],
      ["diaphragm", "Moves air in and out of the lungs"], ["gall bladder", "Stores bile"],
      ["pituitary gland", "Controls other hormone glands"], ["retina", "Detects light in the eye"],
      ["cochlea", "Converts sound vibrations into nerve signals"], ["xylem", "Carries water up a plant"],
    ] as [string, string][],
...
const geographyFacts: [string, string][] = [
  ...
  ["What is the average weather over many years called?", "Climate"], ["What map line joins points of equal height?", "A contour line"],
  ["What causes tides?", "The gravitational pull of the moon"], ["What is a ring-shaped coral island called?", "An atoll"],
...
const geographyBank = () => [
  ...fromMap(geographyFacts, (k) => k, (_k, v) => `The correct answer is: ${v}.`),
  ...fromMap(
    [
...
export const secondarySections: SectionDefinition[] = [
  { id: "secondary-algebra", name: "Algebra", description: "Linear equations, expressions, quadratics and graphs.", difficulty: "Medium", build: algebraBank },
  { id: "secondary-geometry", name: "Geometry & Mensuration", description: "Area, perimeter, angles and circles.", difficulty: "Medium", build: geometryBank },
  { id: "secondary-chemistry", name: "Chemistry", description: "Elements, bonding, acids, bases and reactions.", difficulty: "Medium", build: chemistryBank },
  { id: "secondary-physics", name: "Physics", description: "Forces, energy, waves, electricity and SI units.", difficulty: "Medium", build: physicsBank },
  { id: "secondary-biology", name: "Biology", description: "Cells, human systems, plants, genetics and ecology.", difficulty: "Medium", build: biologyBank },
  { id: "secondary-geography", name: "Geography", description: "Physical geography, maps, climate and world capitals.", difficulty: "Medium", build: geographyBank },
];
